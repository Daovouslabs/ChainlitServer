...
async def connection_successful(sid):
    ...
    if isinstance(context.session.auth_client, BaseAuthClient) and config.project.database in [
        "local",
        "custom",
    ]:
        flag, user_info = await context.session.db_client.create_user(context.session.auth_client.user_infos)
